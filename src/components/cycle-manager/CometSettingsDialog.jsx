...
export default function CometSettingsDialog({ open, onOpenChange }) {
  ...
  useEffect(() => {
    ...
    if (data) {
      ...
      // Fallback to cycle_creation_data only when enabled_attributes doesn't have the value
  ...
  const handleSave = async () => {
    try {
      ...
      // Also update Experience Design in cycle_creation_data if duration or source_alignment changed
      ...
      if (response && response?.autoSaveComet) {
        ...
        try {
          ...
          console.error(
            "Failed to publish cycle from settings dialog:",
            publishError,
          );
          toast.error("Failed to publish cycle. Please try again.");
        }
      }
      onOpenChange(false);
    } catch (error) {
          ...
          <div className="md:w-60 lg:w-64 xl:w-72 bg-white rounded-lg border-gray-200 p-2 space-y-2 overflow-x-auto md:overflow-x-visible">
            <div className="flex md:flex-col gap-2 min-w-max md:min-w-0">
              ...
              >
                ...
                />
                <span className="hidden md:inline">Cycle Info</span>
