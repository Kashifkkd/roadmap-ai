...
export default function GenerateStepImageButton({
  ...
  // Fetch status on mount (to check for failures) and poll if enqueued
  useEffect(() => {
    if (!sessionId || !chapterUid || !stepUid) return;
    
    // Always check status once to catch any historical failures
    fetchStepStatusSilent();

    // If enqueued and not yet complete, poll every 10s
    if (isEnqueued && !stepStatus?.images?.is_complete) {
